...
### [2026-06-22] AWS Bedrock 多区域并发 + Global 跨区域推理（feat，**未提交**）
- **需求/方案**：AWS(type 33)渠道支持「填一对凭证 → 多选区域 → 并发测试可用性 → 一键写回 `AK|SK|region` 多行 → 复用批量+密钥聚合」叠加多区配额；另加「强制使用 Global 跨区域推理」开关(模型 ID 走 `global.` 前缀)。设计 `specs/2026-06-22-aws-bedrock-multi-region-design.md`，实施计划 `plans/2026-06-22-aws-bedrock-multi-region.md`。
- **后端**：① `dto/channel_settings.go` ChannelOtherSettings 加 `AwsForceGlobal`(omitempty，旧 settings 反序列化为零值 false，无迁移)；② 新建 `relay/channel/aws/region_probe.go` 导出 `BuildBedrockRuntimeClient/ResolveModelIDForRegion/ProbeRegionAvailability/ClassifyRegionProbe`，`newAwsClient` 重构为复用(DRY，行为等价)；③ `relay-aws.go doAwsClientRequest` 加 Global 分支(开→`global.`无条件前缀绕过白名单，关→原 us/eu/apac 逻辑不变)；④ 新建 `controller/channel-aws.go TestAwsRegions`(bounded 并发 8、单区 ctx 超时 10s、200/429→ok、403/400→not ok，**凭证绝不落日志**)；⑤ `router/api-router.go` channelRoute 组注册 `POST /channel/aws/test_regions`(自带 AdminAuth)。
- **TDD**：新增 `region_probe_test.go`(3 纯函数)+`relay_aws_global_test.go`(table：global on→`global.anthropic.claude-opus-4-6-v1`、off→`us./eu.`)，先红后绿。**TDD 抓到一个真 bug**：重构 newAwsClient 时把 region 误当 apiKey 传(签名顺序 keyType,ak,sk,apiKey,region；3 段 AK/SK 模式 region 是第 5 参)，被既有 `anthropic-beta` 测试照出并修复；同步改正计划文档。aws 包 8/8 passed。
- **前端(仅 web/classic，未碰 web/default)**：① `constants/channel.constants.js` 加 `AWS_BEDROCK_REGIONS`(19 区 us/eu/ap)+ `index.js` re-export；② `EditChannelModal.jsx`：Global `Form.Switch` + 区域多选 `Select`(测试后 label 追加 `✓320ms`/`✗...`)+「测试可用区域」「生成密钥(写回密钥框)」两按钮 + 3 handler(`pickAwsTestModel` 仅 Claude/便宜优先 haiku>sonnet>opus、`handleTestAwsRegions` 全量并发探测回填默认选中可用、`handleGenerateAwsKeys` 去重写回 key 字段复用批量/聚合)；settings 存/cleanup/回填(3 处)接 `aws_force_global`；③ i18n `en.json`+`zh-CN.json` 各加 11 条(zh 为 identity，与项目约定一致)。
- **验证**：`go build ./...`✅、aws 包 `go test` 8/8✅、`go vet`✅(controller 仅 pre-existing `TestListModelsTokenLimitIncludesTieredBillingModel` 失败——移走我的新文件后仍失败，已证非本次引入)；classic `bunx eslint`(我改的 3 文件 clean，仓库 85 个 missing-header 为存量)、`bun run build` exit 0。
...
### [2026-06-24] 生产发版 v2026.06.24.1（V15+V16+V17，/tke-release 全流程）
- **提交**：用户指令 `提交` → 特性分支 `feat/aws-channel-ux-v15-v17` commit `7213d8b4`(7 文件 +165/-85)；用户指令 `/tke-release` → fast-forward 合并 main、另补 `7985add5` docs(deploy) 收旧 v2026.06.22.1 发版报告。
- **/tke-release 6 Phase 全过**：P1 预检(main / go build / go test 仅白名单内 pre-existing 失败 / 版本 v2026.06.24.1) → P2 push main+tag、Actions run 28089595305 `completed/success`(JSON 二次校验) → P3 master 1/1 + slave 2/2 滚动收敛、健康 success:true、**无 panic**(健康检查 skill 模板 `grep '\s'` 在 macOS BSD grep 假失败，已用原始响应确认健康) → P4 冒烟(status✅ / relay OpenAI+Anthropic 基线 model_not_found 非崩溃，smoke token default 组无渠道，与历次一致) → P6 监控 6 轮真实 5xx=0(仅 P6 起点前 2×503 user2 自测 + 1×404 user1 无关客户端错误)。
- **结果**：✅ prod=v2026.06.24.1(master+slave v2026.06.22.1→v2026.06.24.1)。发版报告 `docs/deploy/report/2026-06-24-tokenki-prod-v2026.06.24.1.md`。
- **覆盖说明**：V15-V17 渠道管理改动需管理员/供应商会话，prod relay smoke token 无法触发，已在本地隔离栈端到端实测覆盖(详见上方 V15/V16/V17 条目)。
- **提交状态**：代码已 commit(7213d8b4)+push、tag 已推、prod 已部署。**本发版报告 + 本 WORKLOG 条目尚未 commit**(等用户指令)。

### [2026-09-17] 上游必要修复同步 + 最新模型列表（分支 `chore/upstream-sync-2026-09`，**未合入 main / 未 push**）
- **需求(用户)**：对比上游 QuantumNous/new-api 把"必要的更新"拉下来合并，并确保支持最新模型列表；Azure Claude 不做专用类型，确认 Anthropic Claude 渠道填 `https://<resource>.services.ai.azure.com/anthropic` 即可跑（适配器拼 `/v1/messages` + `x-api-key` + `anthropic-version`，与微软文档一致）。
- **前置分析**：本地自 `6f415428`(2026-06-11) 分叉，上游领先 500 提交(rc.11→rc.37)。上游 7 月 20 日 `31d70fca3` 删掉整个 web/classic 并把鉴权改为无状态 token，rc.26 起额度列 int32→int64 迁移、rc.34 密码 Argon2id/Telegram 统一 OAuth，官方仍标"不推荐生产"——**不能整体 merge**，采用按主题 `cherry-pick -x`。用 `git merge-tree --write-tree --merge-base=<c>^ HEAD <c>` 只读预演每个提交的冲突面。安全通告核对：6 月后 7 个 GHSA 中 5 个本地已修，**未修 2 个**：GHSA-8r8v-xf7q-rcpr(critical，计费数量参数整数溢出→负额度自充值)、GHSA-j6gc-4893-qwmp(high，PUT /api/user/self 回写全量 User 覆盖 Redis 额度)。
- **落地(25 个提交)**：
  - 安全 GHSA-8r8v：`d0bd8aac7`(新增 common/quota_math.go 饱和转换+数量上界) → `c9943d37a` → 依赖链 `bae799ccb`(QuotaClamp 审计) → `3fbad6a72` → `621927f71`(预扣费饱和直接 400) → `d9595831b`；上游 `fc1259f58`(PriceData.otherRatios 私有化重构)未整拿，只移植 `HasOtherRatio/OtherRatioMultiplier/ApplyOtherRatiosToFloat` 三个方法(types/price_data.go)保持字段导出，Tokenki 的 controller/relay.go、task_billing.go、relay_task.go 调用不受影响。
  - 安全 GHSA-j6gc：`dfc0d6324`(UpdateUserSetting 单列更新 + updateUserCache 仅刷非额度字段 + Update 用 current 快照并 Omit quota/used_quota/request_count) → `0cd9dc85e`(UpdateUserAccessToken 单列、inviteUser 原子 aff_*、Omit 扩到 access_token/aff_*、/api/user/token 加 CriticalRateLimit) → `bfddc5fea`(查询 Omit access_token)。本地无 AuthVersion/lockForUpdate，按本地写法保留 `tx.Set("gorm:query_option","FOR UPDATE")`。
  - 稳定性：`986d90ae0` 优雅关闭；`d3874db61` 限流器；`b518d0033` ResponseHeaderTimeout(手工移植为 `applyResponseHeaderTimeout()` 挂到默认/HTTP 代理/SOCKS5 三个 transport，`RELAY_RESPONSE_HEADER_TIMEOUT` 默认 1800s，附单测)；`1751f43ee` SQLite WAL(保留 `tokenki.db` 文件名)；`66031a09d` PG `PrepareStmt:false`；`d6b5ce99d` Request.GetBody 支持 HTTP/2 stream reset 透明重试(保留本地 client 选择逻辑，采纳上游"relay 不跟随 3xx，原样返回"的 shallow-copy 写法)；`bd585d78e` Bedrock 客户端断开即 cancel 上游并 SkipRetry(上游 relaykit 路径的测试改到本地 `dto`，BillingUsage 断言改为顶层 PromptTokens/CompletionTokens≥1)；`4442bb302` Claude 空 tools 不注入(手工移植到 relay/channel/claude，TDD)。
  - 最新模型：cherry-pick `2f5f6ba84`(gpt-5.5+ 完成倍率放开为 6x 默认)、`16bfae175`(Realtime GA)、`8b41defbe`(gemini-3-pro-image / 3.1-flash-image GA)；手工按官方价补默认定价与内置列表：gpt-5.5、gpt-5.6-sol/terra/luna、gpt-6-astra($10/$50，>272K 长上下文档需管理员表达式定价)、claude-opus-5($5/$25)、claude-sonnet-5($2/$10)、claude-sonnet-4-6($3/$15)、claude-fable-5 / -5-1($10/$50，5.1 缓存读 0.025x)、claude-mythos-5 / -5-1、claude-haiku-4-5 别名；完成倍率规则泛化到所有 claude-sonnet-/opus-/fable-/mythos-(5x) 与 gpt-6(5x)；OpenAI/Codex 内置列表补齐；AWS Bedrock 与 Vertex 的 Claude 5 官方 ID 映射补齐。`setting/ratio_setting/latest_models_test.go` 钉住整张表。
  - Claude 5 能力适配：新增 `setting/reasoning/claude.go` `ClaudeCapabilitiesFor()/NormalizeClaudeEffort()`(对齐上游 relaykit reasoning/claude.go)，替换 relay/channel/claude 与 relay/claude_handler.go 里 opus-4-6/4-7/4-8 前缀白名单：Claude 5、Fable、Mythos 的 `-low/-medium/-high/-xhigh/-max/-thinking` 后缀走 adaptive + effort，strict-sampling 模型清 temperature/top_p/top_k，Fable 无 max→xhigh、4.6 无 xhigh→max。TDD 3 个新用例。
...
### [2026-09-17] 观察员角色 + 供应商端隐藏售价 + 角色下拉框（分支 `feat/viewer-role-supplier-price-hiding`，已 commit，**未 push / 未发版**）
- **需求(用户)**：① 新增"观察员"角色给客户核对资源：能看全部渠道但不可编辑、看不到成本价/密钥，能看消耗与余额、全平台使用日志，能建令牌、调用、有钱包；② 用户问"Azure Claude 分组倍率 3.3 供应商能否看到/影响结算"→ 核实结算 `official_usd = quota ÷ (倍率×QuotaPerUnit)` 不受影响，但供应商日志接口原样返回 `quota` 与 `other.group_ratio`，统计为 SUM(quota)——**现网泄露售价**，一并修；③ 用户角色改为编辑弹窗下拉框，删掉"提升/降级"。设计 `docs/superpowers/specs/2026-09-17-tokenki-viewer-role-supplier-price-hiding-design.md`，计划 `docs/superpowers/plans/2026-09-17-tokenki-viewer-role-supplier-price-hiding.md`。
- **后端**：`common.RoleViewerUser=3`（`IsValidateRole` 收录）；`middleware/auth.go` 抽 `authHelperWithRoleCheck(allow)` 谓词版，`authHelper` 变薄包装，新增 `ViewerAuth`（role==3 或 ≥admin，精确匹配防供应商越权）；`/api/viewer/{channel/,channel/search,channel/models,groups,log/,log/stat}` 全 GET；`controller/viewer_channel.go` 白名单 DTO（id/name/type/status/models/group/response_time/test_time/created_time/used_quota/balance/balance_updated_time/channel_info{is_multi_key,multi_key_size}/tag），`listChannelsCore/searchChannelsCore` 改为 `channelListOptions{forceSupplierId,viewer}`；`controller/viewer_log.go` `blankViewerLog`（抹 username/token_name/ip/user_id/token_id/official_usd/cost_price_snapshot/admin_info/stream_status，保留 channel_name/quota/group_ratio）；供应商 `blankSellingPrice`（quota=0，剔 group_ratio/user_group_ratio/billing_mode/matched_tier/billing_preference/billing_source/wallet_quota_deducted/subscription_*/admin_info/stream_status），`SupplierLogsStat` 改返 `official_usd`（新 `model.SumSupplierOfficialUsd`）；角色：`controller.validateRoleChange`（只收 1/3/5/10，拒设超管/改超管/改自己，沿用 `canManageTargetRole`），持久化走新 `model.UpdateUserRole`（单列 + `invalidateUserCache`）——**不动 `Edit`**（既有 `TestEdit_DoesNotEscalateRoleStatusQuota` 锁定白名单）；`ManageUser` 删 promote/demote 并加 `default` 拒未知动作；i18n 新键 `user.role_invalid / cannot_change_root_role / cannot_change_own_role`（en/zh-CN/zh-TW）。
- **前端(仅 classic)**：`isViewer()`/`ViewerRoute`；`ChannelsPage mode='viewer'`（apiBase `/api/viewer/channel`，隐藏 成本价/应收款/创建者/优先级/权重/操作列、添加/批量/标签模式/批量开关、行选择、所有写操作弹窗，余额不可点刷）；`useLogsData({mode})` 加 viewer 分支（隐藏用户/令牌/IP/应付/重试列，渠道列可见，渠道 ID 筛选可用）；供应商模式隐藏"花费"列、统计显示"官方计费 $x"；新页 `/console/viewer/channels`、`/console/viewer/logs`，侧栏"观察员"分区；用户表删"提升/降级"按钮与两个弹窗文件，`EditUserModal` 加角色下拉（超管可选 1/3/5/10，管理员 1/3/5，目标为超管或自己时禁用），`renderRole` 加"观察员"紫标。
- **验证**：新增 Go 测试 9 个文件（role/ViewerAuth 矩阵/白名单序列化/日志脱敏/售价抹除/官方价汇总/角色规则表/单列更新）全过；`go test ./common ./middleware ./controller ./model` 仅剩基线白名单 `TestListModelsTokenLimitIncludesTieredBillingModel`；`bun run build`+prettier+eslint 过。**本地容器端到端**（交叉编译 arm64 → docker cp `/tokenki` → restart，注册 3 个 zzt_* 测试号并用 SQL 设 role 3/10）：观察员 `/api/channel/` 403、`/api/viewer/channel/` 200 且响应无 cost_price/base_url/key/override/remark/supplier_name 任一键、`/api/viewer/log/` username/token_name/ip 空、official_usd=0；供应商进 `/api/viewer/*` 403；造一条含 group_ratio=3.3/quota=3300 的供应商日志：供应商侧 quota=0、other 仅剩 model_ratio/completion_ratio/frt，stat official_usd=0.5；观察员侧 quota=3300、身份空；管理员 PUT role=3 落库并新会话即时生效，role=10/100/改自己均按预期拒绝，缺省 role 不改，`manage promote` 返回参数错误。测试账号与造数据已清理，容器日志 0 panic。本地容器现运行本分支二进制（版本 `juhe-viewer-<sha>-r2`）。
...
### [2026-09-17] 生产发版 v2026.09.17.4（角色变更无感切换补丁，tke-release 全流程）
- **提交**：用户指令"改完提交并发版" → `15baf7fa7 fix(auth)` 直接提交 main（另 `4add75315 docs(deploy)` 收 .3 发版报告）→ push main → tag v2026.09.17.4。发版前弹窗二次确认。
- **6 Phase 全过**：P1 build / 四包测试仅白名单内 / PG 写路径冒烟 PASS → P2 run 35224606145 watch exit 0 + JSON 二次校验 success（6 分钟），amd64 manifest 就位 → P3 **第一次 set image 遇本机→集群 EOF，未生效（集群仍 .3、健康 200），重连后重试成功**：master 21:12:37 → slave 21:13:16，健康 `success:true`，Pod 0 panic → P4 冒烟 7 条（relay 基线 503 与历次一致；未登录路由 401 JSON；SPA 新包含角色同步标记）→ P6 六轮：503×2 全为冒烟 user_id=2，401×4 全为探测，429/400 既有，真实用户 5xx=0；非 access error 级前后等长窗口 2886 vs 2610；`/api/user/self` 有 2 次真实用户 200（新增启动核对已在线上触发）。
- **结果**：✅ prod = v2026.09.17.4（master+slave .3→.4）。发版报告 `docs/deploy/report/2026-09-17-tokenki-prod-v2026.09.17.4.md`。
- **覆盖说明**：浏览器端 Toast/跳转行为 prod 无账号未点，后端语义已本地真 PG 同会话验证。**建议用户复现原场景确认**。
- **提交状态**：代码已 commit+push、tag 已推、prod 已部署。**本发版报告 + 本 WORKLOG 条目尚未 commit**（等用户指令）。

### [2026-09-18] 排查：供应商待结算金额偏低（根因已定位，**未改码**）
- **现象(用户)**：供应商管理页 Davse 待结算 ¥3,471.85（$1,578.11），但其两条 Anthropic 渠道 op5-1(22)/op5-2(23) 已用 $2943.81 + $2820.79。
- **根因**：`service/supplier_billing.go` `ComputeOfficialUsd` 只算 `(prompt + completion×completion_ratio)×model_ratio`，**完全漏掉 cache_read（×cache_ratio 0.1）与 cache_creation（×1.25，含 5m/1h 拆分）**。Claude 原生口径（`other.usage_semantic=anthropic`，v2026.06.24.1 起即如此）下 `summary.PromptTokens` 仅为非缓存输入，缓存 token 被整体丢弃；OpenAI 口径下 PromptTokens 含缓存，则反过来把缓存按全价计（高估）。另漏 tool 附加费/图像/音频/OtherRatios（本例无）。P4 上线（2026-06-16）起即存在，非 9 月上游同步引入。
- **证据**：CLS 拉取 9/17 08:00–16:24 UTC 渠道 22/23 全部 28,468 条 `record consume log`（脚本 `/tmp/tk_official_audit.py`，条数与 CLS count 精确一致）：已用 $5,844.94；记录 official_usd $1,604.17；按 token 含缓存重算 $5,842.34，按 quota÷group_ratio 重算 $5,842.37（两法互证差 $0.03）；**记录值仅为真值 27.5%**；28,464/28,468 条与漏算公式逐条吻合；token 构成 prompt 7.99M / cache_read 1,824M / cache_write 532M / completion 62.6M（缓存占输入 99.7%）；99.9% 流量走 `Claude官key` 分组 group_ratio=1，故该供应商「已用 $」≈ 官方价。少量 Azure Claude 3.3/3.4 分组流量 ~$1.7。
- **影响**：按成本价 ¥2.20，该供应商待结算少记约 $4,238 ≈ ¥9,300；其他供应商历史已结算/待结算账单若含 Claude 缓存流量同样偏低（OpenAI 口径含缓存则偏高），需复核。9/17 条目中「结算 official_usd = quota ÷ (倍率×QuotaPerUnit)」的表述仅对音频/任务/MJ 路径成立，文本路径实为 token 公式（「分组倍率不影响结算」结论仍成立）。
- **建议修法(待用户定)**：① 文本路径官方价与 quota 同源：在 `calculateTextQuotaSummary` 用同一 decimal 管线（含缓存/图像/音频/tool 附加费/OtherRatios）按 groupRatio=1 产出 `OfficialUsd`；或直接 `OfficialUsdFromQuota(summary.Quota, summary.GroupRatio)`（与其他路径一致，但免费分组 group_ratio=0 时得 0）。补回归测试（Claude 口径带缓存 / OpenAI 口径带缓存 / 5m-1h 拆分）。② 存量回填：`settlement_id=0` 的供应商渠道日志按 `other` 内 token 与倍率重算 official_usd（prod PG 一条 UPDATE 或 Go 一次性回填，先 dry-run 出差额）；已结算单不回填（沿用 6/14「不回填存量」决策）但需向供应商说明差额。
- **CLS 经验**：tccli 走默认端点 `cls.tencentcloudapi.com` 本机持续 SSL EOF，改 `--endpoint cls.ap-tokyo.tencentcloudapi.com` 稳定；SQL 分析不能引用 `msg`（不在统计索引），只能拉原始日志本地算；单查询窗口最多 1 万条，需按时间切块 + Context 翻页。
- **验证**：仅调研，未改码、未 commit。

### [2026-09-18] 修复：供应商官方价漏算缓存 token + 历史日志回填工具（代码 + 测试 + 本地 PG 冒烟，**未提交**）
- **做了什么**：① 文本路径官方价改为与 quota 同源：`calculateTextQuotaSummary` 新增 `summary.OfficialUsd`，倍率 / 按次两条分支都由同一 decimal 管线 ÷ groupRatio 得出（天然含缓存读写、图像、音频、tool 附加费、附加倍率），groupRatio=0 免费分组退回 token 口径 ×modelRatio×OtherRatios，total_tokens=0 归零；`PostTextConsumeQuota` 改用 `summary.OfficialUsd`（分层表达式计费时用 `OfficialUsdFromQuota(quota, groupRatio)`）。删除漏算的 `ComputeOfficialUsd`。② `common.OfficialUsdFromQuota` 抽为唯一实现，`service.OfficialUsdFromQuota` 委托之，model 层回填共用。③ 新增 `model.BackfillUnsettledOfficialUsd(afterId, limit, dryRun)`：只扫供应商渠道 + type=2 + settlement_id=0 的日志，按 `quota ÷ other.group_ratio ÷ QuotaPerUnit` 重算，other 缺 group_ratio 跳过并计数，|Δ|<1e-9 视为不变，按 id 游标分页，UPDATE 再带 settlement_id=0 防并发打包；Go 侧解析 JSON、无 JOIN，三库兼容。④ 超管接口 `POST /api/admin/settlement/backfill-official-usd`（RootAuth）：body `{dry_run(缺省 true), after_id, limit(缺省 5000、上限 20000)}`；apply 时按供应商各记一条 `action=backfill` 资金账本（official_usd/computed_cny=差额，remark 带条数与游标）并 SysLog。
- **改了哪些文件**：`service/text_quota.go`、`service/supplier_billing.go`（重写）、`service/supplier_billing_test.go`（删 3 个旧公式测试）、`common/quota_math.go`、`model/log_official_usd_backfill.go`（新）、`controller/settlement_backfill.go`（新）、`router/api-router.go`；测试新增 `service/text_quota_official_usd_test.go`（7 个 summary 级 + 1 个 PostTextConsumeQuota 端到端）、`model/log_official_usd_backfill_test.go`（4）、`controller/admin_backfill_official_usd_test.go`（3）。
- **验证**：TDD 先红后绿（三处均先因符号缺失编译失败）；`go test ./common ./service ./model ./controller` 仅剩基线白名单 `TestListModelsTokenLimitIncludesTieredBillingModel`；`go build ./...`、`go vet` 四包干净（common 两条 custom-event 既有告警）。生产真实日志用例（prompt 2 / completion 546 / cache_read 78919 / cache_write 578）官方价 $0.01366 → $0.056732（= quota 28366 / 500000），分组倍率 3.3 时官方价不变。**本地一次性 PG 冒烟**（`/tmp/pg-backfill-smoke.sh`，postgres:15 + darwin 二进制 + /api/setup 登录，造 4 条日志）全过：dry-run 不写库不记账 → apply 只改未结算供应商日志 0.004785→0.130778，已打包 / 无 group_ratio / 非供应商渠道不动，账本 `backfill|777|Δ$0.125993|Δ¥0.277185` → 二次 apply 幂等 updated=0 且不重复记账 → `/api/supplier/summary` 待结算随之变化；应用日志无 panic / 22P02。
- **上线步骤（待用户指令）**：commit → tke-release 发版 → 超管 `POST /api/admin/settlement/backfill-official-usd` body `{}` 看 dry-run 差额（per_supplier）→ body `{"dry_run":false,"limit":5000}` 循环到 `done=true`（用返回 next_after_id 作下一轮 after_id；每轮各供应商记一条账本）。届时若存在「已申请」账单需先撤销、回填、再重新发起；已结算单不动。
- **提交状态**：**未 commit、未发版**（等用户指令）。

### [2026-09-18] 生产发版 v2026.09.18.1（供应商官方价含缓存 token + 未结算日志回填接口，tke-release 全流程）
- **提交**：用户指令"提交发版" → 弹窗确认 prod + 版本号 + 两个 commit → `0d9b5c135 fix(billing)`（代码+测试）、`0db45eb66 docs(deploy)`（v2026.09.17.4 报告 + WORKLOG）→ push main → tag v2026.09.18.1。
- **6 Phase 全过**：P1 build / 四包测试仅白名单 / **Step 1.1b PG 写路径冒烟 PASS** → P2 run 35252684149 watch exit 0 + JSON 二次校验 completed/success（约 5 分钟）→ P3 master 01:31:36 → slave 01:32:16 收敛，健康 200 `success:true`，3 Pod Running restarts=0，migration 无 error，无 panic → P4 冒烟 7 条（relay 基线 503 与历次一致：冒烟 token 分组无渠道；新接口/供应商汇总未登录 401 JSON；SPA 200）+ **修复本体生产验证**：CLS 部署后渠道 22/23 消费日志，新 Pod 183/183 条 `official_usd == quota ÷ group_ratio ÷ 500000`，45 条不一致全部来自优雅退出中的老 Pod（旧公式），老 Pod 01:36 前全部退出 → P6 两轮：429×192（发版前等长窗口 197）、401×5（3 条 /v1/models 探测 + 2 条我的鉴权冒烟）、503×3 全为 user_id=2 冒烟，真实用户 5xx=0；非 access error 级 2884 vs 3165 量级一致；panic/SQLSTATE 0。
- **结果**：✅ prod = v2026.09.18.1（master+slave .4→.18.1）。发版报告 `docs/deploy/report/2026-09-18-tokenki-prod-v2026.09.18.1.md`。
- **下一步**：线上回填校准（需超管 access token）：dry-run → apply 循环到 done=true → 核对供应商管理页待结算。
- **提交状态**：代码已 commit+push、tag 已推、prod 已部署。**本发版报告 + 本 WORKLOG 条目尚未 commit**（等用户指令）。

### [2026-09-18 01:45–02:06] 线上回填校准：Davse 待结算从 $2,518 校准到 $7,562（超管 access token 经用户提供，已用完建议重置）
- **发现**：dry-run 前线上状态 = 待结算 $252.80/2,333 条 + **「已申请」账单 #4**（Davse，01:11 手动发起，32,376 条，旧公式 $1,834.23 / ¥4,035.31）。回填只动 settlement_id=0 日志，#4 需先撤销。弹窗询问后**用户手动撤销 #4**（status=3 已核实）。
- **执行**（`/tmp/tk_backfill_drive.py`，Authorization=系统访问令牌 + New-Api-User=1，admin role=100）：apply 8 批 × 5000：scanned 36,661 / updated 36,103 / unchanged 558 / skipped 0；Davse old $2,518.17 → new $7,554.16（Δ +$5,035.99 / **+¥11,079.17**），每批各落一条 `action=backfill` 账本。随后 `POST /api/admin/settlement/initiate {supplier_id:17}` → **新账单 #5**：$7,561.93 / **¥16,636.25**，36,702 条，周期 09-17 16:23:54 → 09-18 02:05:17，状态 已申请（待超管确认实付）。校准后汇总：待结算 $0.67/5 条（发起后新到日志）、已申请 1 单 $7,561.93、已结算 1 单 $0.43。
- **交叉核对**：Davse 两渠道 used_quota 合计 $10,038.63（含分组倍率售价口径；早期 Claude官key 倍率 1、后段 3.4），官方价 $7,562 量级相符（CLS 08:00–16:24Z 段 $5,842 @倍率 1 + 后段 $4,194÷3.4≈$1,234 ≈ $7,076，差额为倍率切换前后混合流量）。
- **注意**：老 Pod 在 01:36 前全部退出，此后无旧公式日志；回填后新增日志均由新版本记账，无需二次回填。用户提供的超管系统访问令牌已出现在本会话中，**建议到个人设置重新生成**。
- **提交状态**：发版报告 `docs/deploy/report/2026-09-18-tokenki-prod-v2026.09.18.1.md` + 本 WORKLOG 两条（发版、校准）**未 commit**（等用户指令）。

### [2026-09-18 02:10–02:50] 排查「已用 $10.3k vs 应结 $7.6k」+ 修复「待结算不跟随最新成本价」（代码 + 测试 + PG 冒烟，**未提交**）
- **问题①（不是 bug）**：用户见 Davse 两渠道「已用」$5,264.01 + $5,048.58 ≈ $10,312，而结算约 $7,562。核对 prod 配置：`GroupRatio` Azure Claude=3.4 / Azure Claued 3.2=3.2 / Azure Claude 3.3=3.3 / Claude官key=1，`Price`=7.3（¥/$），`DisplayInCurrencyEnabled`=true → 「$」是真实美元、分组倍率是加价倍率。渠道「已用」= Σ 用户被扣额度（含倍率）；供应商官方价 = 额度 ÷ 倍率。CLS 分段：08:00–16:24Z 全走 Claude官key(×1) 已用 $5,845 = 官方 $5,842；16:24–18:15Z Azure Claude(×3.4) 已用 $4,110 → 官方 $1,209，Claude官key 已用 $691 = 官方 $691。合计官方 ≈ $7,742（含 02:05 后新增），与账单 #5 $7,561.93 + 待结算一致。差额 ≈ $2,900 全部是 Azure Claude 分组 3.4× 加价部分，属平台毛利，不应付给供应商。管理端渠道列表目前只有供应商模式显示「应收款」列，管理员看不到官方价/应收，容易与「已用」混淆——可选后续：管理员模式也展示。
- **问题②（已修）**：成本价 ¥2.2 → ¥2.0 后待结算仍按 2.2。根因是 2026-06-14 安全整改「成交价逐条冻结在 `cost_price_snapshot`，结算不活取现价」（防供应商结算前抬价套现）；用户明确要求结算前改价按最新价。**新口径**：未结算日志的成交价随渠道**当前**成本价重定价，打包进账单（settlement_id≠0）后冻结；每次重定价记 `action=reprice` 资金账本（official_usd=受影响官方价合计，computed_cny=应付差额，remark 含渠道/条数/前后金额）留痕。
- **改了哪些文件**：`model/log_reprice.go`（新：`RepriceUnsettledLogs(channelId, supplierId, newPrice, dryRun)`、`RepriceAllUnsettledLogsToCurrent(dryRun)`）、`controller/settlement_reprice.go`（新：`AdminRepriceUnsettled` 接口、`repriceChannelIfCostChanged` hook、`recordRepriceLedger`）、`controller/channel.go`（管理员 `UpdateChannel` 保存后回读渠道并触发 hook）、`controller/supplier_channel.go`（供应商 `SupplierUpdateChannel` 同上，OperatorIsAdmin=false）、`router/api-router.go`（`POST /api/admin/settlement/reprice-unsettled`，RootAuth，缺省 dry_run）、`model/{settlement_query,supplier_stats,settlement}.go` 注释改为新口径。测试：`model/log_reprice_test.go`（4）、`controller/settlement_reprice_test.go`（4：接口 dry-run/apply+账本、供应商改价触发重定价+账本、成本价未变不动）。
- **验证**：TDD 先红后绿；`go test ./model/... ./service/... ./controller/... ./logger/...` 仅白名单 `TestListModelsTokenLimitIncludesTieredBillingModel`；build/vet/gofmt 干净。**一次性 PG 冒烟** `/tmp/pg-reprice-smoke.sh` 全过：管理员 PUT 改价 2.2→2.0 后未结算日志 2.2→2.0、已打包保持 2.2、账本 `reprice|777|admin|$1.0|¥-0.2`；对齐接口 dry-run 不写库、apply 生效、二次账本；`/api/supplier/summary` 待结算按新价。
- **风险提示（已告知用户）**：供应商可自改成本价，抬价后待结算立刻变大——靠超管确认实付把关 + reprice 账本可追溯；竞价调度按成本价择优，抬价会失去流量，是天然约束。
- **上线后待办**：① `POST /api/admin/settlement/reprice-unsettled` dry-run → apply，把 hook 上线前已改价（2.2→2.0）的存量未结算日志对齐；② 账单 #5（¥16,636.25 按 2.2 冻结）若要按 2.0（≈¥15,123.87）需撤销 → 对齐 → 重新发起，等用户定。
- **提交状态**：**未 commit、未发版**（等用户指令）。

### [2026-09-18 02:15–03:00] 供应商可见性全局审计 + 渠道表「已用/剩余」「已消耗/应收款」按角色分列（代码 + 测试 + PG 冒烟，随 v2026.09.18.2 发版）
- **需求(用户)**：渠道「已用」是含倍率的售价口径，供应商不得看到售价/倍率/含倍率金额；「已用/剩余」只给管理员与观察员；新增「已消耗/应收款」（已消耗=官方计价不含倍率的累计消耗 $，应收款=已消耗×成本价 ¥）给管理员与供应商，观察员与其他角色不可见；供应商只能看自己渠道的已消耗与应收款；全局检查后实现、测试、发布、核数。
- **审计结论**：泄露点 ① `/api/supplier/channel/`（列表/搜索/详情）原样返回 `used_quota`（含倍率），且 classic 供应商模式显示「已用/剩余」列；② `/api/pricing`（模型广场）对所有登录用户下发 `group_ratio`（Azure Claude 3.4 等）与模型售价，供应商可直接访问 `/pricing`。其余安全：供应商日志接口 9/17 已抹 quota/group_ratio；`/api/supplier/self/realtime` 只回 rpm/tpm（Stat.Quota 未下发）；dashboard/ranking/pending/settlement 全走 official_usd；观察员渠道白名单 DTO 不含 official_usd/receivable/cost_price（既有测试锁定）；市场价按 D7 允许看各分组最低成本价。未改：供应商账号作为普通用户可经 URL 进 `/console/log` 看其本人作为客户的消费（菜单已隐藏），属本人消费非平台售价，不在本次范围。
- **后端**：`model.GetTotalReceivableByChannels`（累计 Σ official_usd×cost_price_snapshot，新）；`controller/supplier_channel.go` `backfillSupplierUnsettled`→`backfillSupplierConsumption`（改为累计口径、只查 supplier_id>0 渠道，管理员与供应商列表/搜索都回填），新增 `hideSellingPriceFromSupplier`（供应商列表/搜索/详情 used_quota=0）；`controller/channel.go` list/search core 接入；`controller/pricing.go` 供应商角色返回空定价/空倍率/空可用分组；`model/main.go` 导出 `InitColNames()`（controller 内存库测试用，SearchChannels 拼原生列名）；`model/channel.go` 字段注释改为累计口径。
- **前端(classic)**：`useChannelsData.jsx` 新增 `COLUMN_KEYS.CONSUMPTION`（缺省可见）、标签聚合行累加 official_usd/receivable；`ChannelsColumnDefs.jsx` 新增「已消耗/应收款」列（$ 已消耗 + ¥ 应收款，非供应商渠道显示 -，放在「已用/剩余」之后成本价之前），删掉供应商模式旧的未结算「应收款」列，供应商模式隐藏 BALANCE（已用/剩余），观察员隐藏 CONSUMPTION（VIEWER_HIDDEN 改用新 key）；i18n en/zh-CN 新增 3 键。`bun run build` 过；改动文件 prettier/eslint 干净（仓库其余 149 文件的 prettier 与 85 条 eslint 问题为既有，未动）。
- **验证**：TDD 先红后绿；新增 `controller/channel_consumption_test.go`（5：管理员列表回填累计值且保留 used_quota、供应商列表/搜索抹 used_quota 并回填、供应商详情抹 used_quota、供应商 pricing 全空而普通用户仍有倍率）、`model/supplier_stats_total_receivable_test.go`（1）；既有观察员白名单测试继续锁定新字段不外泄。四包测试仅白名单 `TestListModelsTokenLimitIncludesTieredBillingModel`；build/vet/gofmt 干净。真 PG 冒烟：`/tmp/pg-reprice-smoke.sh`（改价 hook、对齐接口、待结算随新价、管理员列表 已消耗 $2.00/应收款 ¥4.20、pricing 管理员路径正常）与 SOP `pg-write-smoke.sh` 均 PASS。
- **随同发版**：同批含上一条「成本价改价即时生效」（reprice hook + `reprice-unsettled` 接口）。上线后待办：`reprice-unsettled` dry-run→apply 对齐存量；核对供应商管理页待结算与渠道表已消耗/应收款；账单 #5（¥2.2 口径）按用户决定是否撤销重发。
- **提交状态**：待随 v2026.09.18.2 一并 commit/发版（用户已授权"测试后进行发布"，发版前仍按 SOP 弹窗确认）。
